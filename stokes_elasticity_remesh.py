...
def ale_solve(eps_range, mesh, subdomains, bdry, n = 0, sol_eul = None):
    ...
    # (u_f_old, p_f_old, u_s_old, p_s_old, f_0_old, V_0_old, lam_old, lam_p_old, u_a_old, lam_a_old) = block_split(X_old)
    X_0 = BlockFunction(V)
    (u_f_0, p_f_0, u_s_0, p_s_0, f_0_0, V_0_0, lam_0, lam_p_0, u_a_0, lam_a_0) = block_split(X_0)
    ...
    for e in eps_range:
        eps_ale.assign(e)
        print(f'(inner) solving ALE solid problem with eps = {float(eps_ale):.4f}')
...
def eulerian_solver(mesh, subdomains, bdry, U_0, u_s_ale = None):
    ...
    (u_f, p_f, lam, zeta) = block_split(Xf)
    (u_s, p_s, f_s) = block_split(Xs)

    (v_s, q_s, g_s) = block_split(Ys)
    (v_f, q_f, eta, theta) = block_split(Yf)
    ...
    # extract solution components
    (u_f, p_f, lam, zeta) = Xf.block_split()
