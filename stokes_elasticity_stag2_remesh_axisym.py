...
def ale_solve(eps_range, mesh, subdomains, bdry, n=0, sol_eul=None):
    ...
    sigma_s = 1 / eps_ale * (B - I) - p_s * I
    sigma_s = sigma_s * H_1
    # sigma_s = 1 / eps * (F - H) - p_s * H
    ic_s = det(F_1) * (1 + u_s[1] / r) - 1
    ...
    FUN1 = (-inner(sigma_f, grad(v_f)) * r * dx(fluid)
            - J_a * 2 * u_f[1] * v_f[1] / (r + u_a[1]) ** 2 * r * dx(fluid)  # Extra terms grad(u_f)
            + J_a * p_f * v_f[1] / r * (r / (r + u_a[1])) * r * dx(fluid)  # Extra terms from p_f
            + inner(lam("+"), v_f("+")) * r("+") * dS)
    FUN2 = ic_f * q_f * r * dx(fluid) + lam_p * q_f * r * dx(fluid)

    # elasticity for the solid
    FUN3 = (-inner(sigma_s, grad(v_s)) * r * dx(solid)
            - 1 / eps_ale * (r + u_s[1]) * v_s[1] / (r - u_s_0[1]) ** 2 * r * dx(solid)  # Extra terms from B*H_1
            + (1 / eps_ale + p_s) * v_s[1] / (r + u_s[1]) * r * dx(solid)  # Extra terms from H_1 / eps_ale & p_s H_1
            + inner(as_vector([f_s, 0]), v_s) * r * dx(solid)
            - inner(lam("-"), v_s("-")) * r("-") * dS)
    ...
    FUN7 = dot(ez, u_s) * g_s * r * dx(solid)
...
def eulerian_solver(mesh, subdomains, bdry, U_0, u_s_ale=None):
    ...
    ic_f = diva(u_f, r)  # diva here

    # solids
    F = inv(I - grad(u_s))
    B = F * F.T
    # sigma_s = 1 / eps_eul * ((B - I) - mr * (inv(B) - I)) - p_s * I
    sigma_s = 1 / eps_eul * (B - I) - p_s * I
    ic_s = det(F) * (1 + u_s[1]/(r-u_s[1])) - 1  # u_s[1]/(r-u_s[1]) from axisymmetric contribution
    ...
    FUN1 = (-inner(sigma_f, grad(v_f)) * r * dx(fluid)
            - 2 * u_f[1] * v_f[1] / r * dx(fluid)  # Extra term from grad(u)
            + p_f * v_f[1] * dx(fluid)  # Extra term from p_f
            + inner(lam("+"), v_f("+")) * r("+") * dS)
    FUN2 = ic_f * q_f * r * dx(fluid) + lam_p * q_f * r * dx(fluid)  # diva(u_f) considered above

    FUN3 = (-inner(sigma_s, grad(v_s)) * r * dx(solid)
            - 1 / eps_eul * r**2 / (r - u_s[1])**2 * v_s[1] * dx(solid)  # Extra term frpm p_s and I
            + (1 / eps_eul + p_s) * v_s[1] * dx(solid)  # Extra term from B
            + inner(as_vector([f_s, 0]), v_s) * r * dx
            - inner(lam("-"), v_s("-")) * r("-") * dS)
    FUN4 = ic_s * q_s * r * dx(solid)  # det(F) considered above

    FUN5 = inner(avg(eta), u_f("+") - as_vector([Constant(U_0), 0])) * r("+") * dS
