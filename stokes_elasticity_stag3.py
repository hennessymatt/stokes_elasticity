...
2. Solve nonlinear elasticity + ALE problem
"""
...
Vs = BlockFunctionSpace(mesh, BlockElement(P2, P1, P0, P2, DGT), restrict = [Os, Os, Os, Of, Sig])
...
(v_s, q_s, g_s, v_a, eta_a) = block_split(Ys)

Xts = BlockTrialFunction(Vs)

Xs = BlockFunction(Vs)
(u_s, p_s, f_s, u_a, lam_a) = block_split(Xs)
...
ac_inlet = DirichletBC(Vs.sub(3), Constant((0,0)), bdry, inlet)
ac_outlet = DirichletBC(Vs.sub(3), Constant((0,0)), bdry, outlet)
ac_fluid_axis = DirichletBC(Vs.sub(3).sub(1), Constant((0)), bdry, fluid_axis)
ac_wall = DirichletBC(Vs.sub(3).sub(1), Constant((0)), bdry, wall)

bc_f = BlockDirichletBC([bc_inlet, bc_outlet, bc_fluid_axis, bc_wall])
bc_s = BlockDirichletBC([bc_solid_axis, ac_inlet, ac_outlet, ac_fluid_axis, ac_wall])
...
FUN3 = -inner(sigma_s, grad(v_s)) * dx(solid) + inner(as_vector([f_s, 0]), v_s) * dx(solid) - inner(lam("-"), v_s("-")) * dS
...
FUN9 = -inner(sigma_a, grad(v_a)) * dx(fluid) - inner(lam_a("-"), v_a("-")) * dS
FUN10 = dot(avg(eta_a), u_s("-") - u_a("+")) * dS


FUN_f = [FUN1, FUN2, FUN5, FUN6, FUN8]
FUN_s = [FUN3, FUN4, FUN7, FUN9, FUN10]
...
(u_s, p_s, f_s, u_a, lam_a) = Xs.block_split()
...
for n in range(1,N):
    ...
    print('solving solid and ALE problem...')
